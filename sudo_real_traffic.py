#!/usr/bin/env python3
"""
Real Traffic Capture with Sudo - Simplified Version
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Sudo Real Traffic Cybersecurity System...")
    ...
    logger.info("Sudo Real Traffic Cybersecurity System started successfully")
    
    yield
    
    # Cleanup
    logger.info("Shutting down Sudo Real Traffic Cybersecurity System...")
    if redis_client:
        redis_client.close()
    logger.info("Sudo Real Traffic Cybersecurity System stopped")
        ...
        try:
            with packet_lock:
                if captured_packets:
                    ...
                    recent_packets = captured_packets[-5:]  # Last 5 packets
...
# Create FastAPI app
app = FastAPI(
    title="Sudo Real Traffic Cybersecurity System",
    description="Real-time network traffic monitoring with sudo privileges",
    version="3.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint"""
    return {
        "message": "Sudo Real Traffic Cybersecurity System",
        "status": "running",
        "version": "3.0.0",
...
async def dashboard():
    ...
    <html>
    <head>
        <title>Sudo Real Traffic Cybersecurity Dashboard</title>
        <style>
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🛡️ Sudo Real Traffic Cybersecurity Dashboard</h1>
                <p>Live network traffic monitoring with elevated privileges</p>
            </div>
            
            <div class="traffic-indicator">
                <h2>{'🟢 REAL TRAFFIC CAPTURE ACTIVE' if real_traffic_enabled else '🟡 SIMULATED DATA MODE'}</h2>
                <p>{'Monitoring actual network traffic from your system' if real_traffic_enabled else 'Using simulated data (real capture requires sudo privileges)'}</p>
            </div>
