#!/usr/bin/env python3
"""
Simplified main entry point for the Generative AI-Based Smart Cybersecurity System
This version works without root privileges and uses simulated data
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Simplified Cybersecurity System...")
    ...
    logger.info("Simplified Cybersecurity System started successfully")
    
    yield
    
    # Cleanup
    logger.info("Shutting down Simplified Cybersecurity System...")
    if redis_client:
        redis_client.close()
    logger.info("Simplified Cybersecurity System stopped")

# Create FastAPI app
app = FastAPI(
    title="Generative AI-Based Smart Cybersecurity System",
    description="Real-time threat detection and response system (Simplified Demo)",
    version="1.0.0",
    lifespan=lifespan
)
...
async def generate_simulated_data():
    """Generate simulated cybersecurity data"""
    global threat_stats
...
async def get_threat_events(limit: int = 20):
    """Get recent threat events"""
    try:
        if redis_client:
            events = redis_client.lrange('threat_events', 0, limit - 1)
            return [json.loads(event) for event in events]
        else:
            # Return simulated threat data
            return [{
                'timestamp': time.time() - i,
                'threat_type': random.choice(['suspicious', 'potential_threat', 'high_threat']),
...
async def get_performance():
    ...
    return {
        "events_per_second": random.uniform(1, 5),
        "threat_detection_rate": random.uniform(0.1, 0.3),
...
async def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            .event-item { padding: 10px; border-bottom: 1px solid #ecf0f1; }
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🛡️ Cybersecurity System Dashboard</h1>
                <p>Real-time threat detection and monitoring (Demo Mode)</p>
            </div>
            ...
            async function loadData() {
                try {
                    ...
                    eventsList.innerHTML = events.map(event => `
                        <div class="event-item" style="border-left: 4px solid ${event.is_threat ? '#e74c3c' : '#27ae60'}; padding: 15px; margin: 10px 0; background: ${event.is_threat ? '#ffeaea' : '#f0fff0'};">
